  ...
  return (
    <>
      ...
      <Stack direction="column" spacing={2} my={1}>
        ...
        <Typography component={Link} href="#caltopo">
          CalTopo Map
        </Typography>
      </Stack>
    </>
...
function AboutCaltopo() {
  return (
    <>
      <Section id="caltopo" variant="h4" title="CalTopo Map">
        <Stack direction={{ md: 'row' }}>
          <Box>
            <Paragraph>The Map Id is expected to be a CalTopo Map Id. Only the Id is required. For example if the Map URL is http://caltopo.com/m/ABC12 then the Id is &quot;ABC12&quot;</Paragraph>
            <Paragraph>When a Map Id has been provided it will be shown as a link on the Mission Detail page and as a button with the CalTopo logo on the Activity tiles, on the home page. Clicking on either the link, or the button will open the map in a new browser tab.</Paragraph>
...
function Section({ id, title, variant, mt, children }: { id?: string; title: string; variant: TypographyVariant; mt?: number; children: React.ReactNode }) {
  return (
    <>
