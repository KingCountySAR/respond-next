...
import { DEFAULT_PLACES, Team, TeamStatus } from '@respond/shared/types/operations';
...
export const TeamStatusSelect: React.FC<TeamStatusSelectProps> = ({ team }) => {
  ...
  const handleChange = async (event: SelectChangeEvent<string>) => {
    ...
    const needsConfirm = team.status !== 'In Base' && team.status !== 'Returning To Base' && team.assignedParticipants.length + team.assignedEquipment.length > 0;
    if (needsConfirm) {
      const confirmed = await confirm({
        prompt: `Disbanding ${team.name} will move remaining members and equipment to ${DEFAULT_PLACES.field}. Continue?`,
        destructive: true,
