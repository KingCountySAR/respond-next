...
import { useGroupCommands, useResourceCommands } from '@respond/hooks/commands';
...
export function DashboardAddGroupButton() {
  ...
  const groupCommands = useGroupCommands(activity.id);
  const { open } = useDialogs();

  const handleAdd = async () => {
    const newGroup = await open(DashboardGroupEditDialog, { activity, group: createNewGroup('') });
    if (newGroup != null) groupCommands.createGroup(newGroup);
  };
...
function GroupTile({ group }: { group: Group }) {
  ...
  const editGroup = async () => {
    ...
    if (result != null) groupCommands.updateGroup(result);
  };
  ...
  const handleExpandedDrop = (item: Participant | EquipmentItem, type: string) => {
    if (type === 'participant') {
      ...
      resources.assignParticipant(item.id, { type: 'group', id: group.id });
  ...
  const handleHeaderDrop = (item: Participant | EquipmentItem, type: string) => {
    if (type === 'participant') {
      ...
      groupCommands.updateGroup({ ...group, leaderId: item.id });
    } else if (type === 'equipment') {
      addEquipment(item as EquipmentItem);
    }
  };
  ...
  const addEquipment = async (equipment: EquipmentItem) => {
    ...
    resources.assignEquipment(equipment, { type: 'group', id: group.id });
  };

  const handleLeaderClear = () => {
    groupCommands.updateGroup({ ...group, leaderId: undefined });
  };
