...
export const OutputForm = ({ children, columns }: { children: ReactNode; columns?: number }) => {
  ...
  const outputFields = childrenArray.map((child, index) => {
    return (
      <Grid key={index} size={{ xs: 12, sm: 12 / (columns ?? 1) }}>
        {child}
      </Grid>
    );
  });
...
const OutputShowMore = ({ children, rows }: { children: ReactNode; rows?: number }) => {
  ...
  useEffect(() => {
    const el = contentElement.current;
    if (!el) return;

    const observer = new ResizeObserver(() => {
      // scrollHeight is the full content height regardless of collapse
      setOverflows(el.scrollHeight > collapsedHeightPixels);
    });
  ...
  const cursorStyle = overflows ? 'pointer' : 'default';
