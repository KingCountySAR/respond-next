...
export type ReplacedType<T extends object, _FROM, TO, K extends [...(keyof T)[]]> = { [K2 in keyof T]: K[number] extends K2 ? TO : T[K2] };

function replacePropertiesType<T extends object, FROM, TO, K extends [...(keyof T)[]]>(obj: T, transform: (from: FROM) => TO, ...keys: K): ReplacedType<T, FROM, TO, K> {
  const ret = {} as ReplacedType<T, FROM, TO, K>;
